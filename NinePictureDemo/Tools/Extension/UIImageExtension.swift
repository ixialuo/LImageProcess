...
extension UIImage {
    ...
    //MARK: - 拆分image，并绘制文字
    func separateImageAndDescripetionAndShape(byX x: Int, andY y: Int, descripetion _descripetion: String?, shapeImage: UIImage?, textColor color: UIColor?) -> [UIImage]? {
        ...
        for i in 0...x {
            for j in 0...y {
                ...
                if let imageRef = CGImageCreateWithImageInRect(CGImage, rect) {
                    ...
                    if let pic = shapeImage {
                        let _xsteppic = pic.size.width*1.0/CGFloat(y)
                        let _ysteppic = pic.size.height*1.0/CGFloat(x)
                        let rectpic = CGRectMake(_xsteppic*CGFloat(j), _ysteppic*CGFloat(i), _xsteppic, _ysteppic)
                        let imageRefpic = CGImageCreateWithImageInRect(pic.CGImage, rectpic)
                        let elementImagepic = UIImage.init(CGImage: imageRefpic!)
                        
                        let imgRect = CGRectMake(0, 0, _xstep, _ystep)
                        UIGraphicsBeginImageContext(imgRect.size)
                        elementImage.drawInRect(imgRect)
                        elementImagepic.drawInRect(imgRect)
                        elementImage = UIGraphicsGetImageFromCurrentImageContext()
                        UIGraphicsEndImageContext()
                    }
